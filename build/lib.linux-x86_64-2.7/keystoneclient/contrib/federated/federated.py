...
def get_uname_pass():
    u_name = raw_input('Enter user name: ')
    password = getpass('Enter password: ')
    return u_name, password

def scrape_hidden_input(html_text):
    form_resp = bs4.BeautifulSoup(html_text)
    post_params = {}
    for input_item in form_resp.select("form")[0].find_all("input"):
        #print input_item.attrs
        if('type' in input_item.attrs and
               (input_item.attrs['type'] == 'hidden' or input_item.attrs['type'] =='text' )) :
            post_params[input_item.attrs['name']] = input_item.attrs['value']
        #print(input_item)
    #print post_params
    form_action = form_resp.select("form")[0].attrs['action']
...
def get_unscoped_token(authentication_endpoint):
    ...
    resp = http_session.get(authentication_endpoint, allow_redirects=True, verify=False)
    if "type=\"submit\">Login</button>" in resp.content:
        a = resp.url.split("/")
        host_name = a[0]+"/"+a[1]+"/"+a[2]
        #get the action url
        auth_resp = bs4.BeautifulSoup(resp.text)
        form_post_url = host_name+auth_resp.select("form")[0].attrs['action']
        username, password = get_uname_pass()
        payload = {'j_username': username, 'j_password': password}
        resp2 = http_session.post(form_post_url, data = payload, allow_redirects=True, verify=False)
        form_action, post_params = scrape_hidden_input(resp2.text)
        #print(resp2.text)
        resp3 = http_session.post(form_action, data = post_params, allow_redirects=True, verify=False)
        #print(resp3.text)
        #print(resp3.headers)
        auth_token = resp3.headers['x-subject-token']
        return auth_token
    print(resp)
